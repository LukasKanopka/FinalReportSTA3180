...
cleaned_cars <- cars_data %>%
  mutate(
    ...
    # Fixed regex: changed [ˆ0-9] to [^0-9]
...
Now we'll remove rows with missing values to ensure complete cases for analysis.
...
Let's explore the data to understand the distributions and relationships between variables.

```{r load_clean_data}
# Load the cleaned data
car_data <- read.csv("cleaned_cars_no_null.csv")

# Convert Accident to factor for proper analysis
car_data$Accident <- factor(car_data$Accident)

# Summary statistics
summary(car_data[,c("Price","Mileage","Horsepower","Model_year")])
```
...
Based on our exploratory analysis, we should identify and remove outliers that could skew our results.
...
The standard linear model shows that all predictors are statistically significant, but the R-squared value is relatively low at around 0.46, indicating moderate explanatory power. Residual plots suggest non-linear patterns.
...
The polynomial model with degree `r best_degree` improves on the standard linear model but still shows some residual issues.
...
tree.mod <- rpart(Price ~ Mileage + Horsepower + Accident + Model_year,
                 data=car_data_clean, method="anova")

# Plot the tree
rpart.plot(tree.mod, extra=1, under=TRUE, cex=0.8,
           main="Decision Tree for Car Price Prediction")
```

The decision tree provides an interpretable model showing the key factors in price determination. Each node shows decision rules and average prices.
...
par(mfrow=c(2,2))
plot(predict(mod_std, car_data_clean), car_data_clean$Price, pch=16, col="blue", alpha=0.5,
     main="Standard Model: Pred vs Actual", xlab="Predicted", ylab="Actual")
abline(0,1, col="red")

plot(log_pred, car_data_clean$Price, pch=16, col="purple", alpha=0.5,
     main="Log Model: Pred vs Actual", xlab="Predicted", ylab="Actual")
abline(0,1, col="red")

plot(predict(mod_poly, car_data_clean), car_data_clean$Price, pch=16, col="green3", alpha=0.5,
     main="Poly Model: Pred vs Actual", xlab="Predicted", ylab="Actual")
abline(0,1, col="red")

plot(rf.pred, car_data_clean$Price, pch=16, col="orange", alpha=0.5,
...
1. **Model Performance**: The Random Forest model outperforms all other models with the highest R-squared and lowest error metrics. The log-transformed linear model also performs well, indicating non-linear relationships between predictors and car prices.
...
6. **Prediction Accuracy**: Our best model (Random Forest) can explain approximately 85-90% of the variation in used car prices using just four variables, which is remarkably good for this type of data.

## Future Work

For future improvements to this analysis, we recommend:

1. Incorporating more categorical variables like car brand, model type, and features
2. Testing more complex ensemble methods like gradient boosting
3. Creating separate models for different car segments (economy, luxury, etc.)
4. Including interaction terms between variables (e.g., Brand × Model_year)
5. Collecting additional data on features like transmission type, fuel type, and condition
